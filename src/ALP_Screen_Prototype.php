...
if( ! class_exists( 'ALP_Screen_Prototype' ) ):

/**
 * Prototype class for administration screens.
 * @since 1.0.0
 */
abstract class ALP_Screen_Prototype {
    /**
     * @var string $slug
     * @since 1.0.0
     */
    ...
    /**
     * <p>Array with screen options. Don't forget that you can use screen options only when {@see ALP_Screen_Prototype::$enable_screen_options} is set on <code>TRUE</code>. You can define them like this:</p>
     * <pre>
     ...
     * <ul>
     *   <li>{@see ALP_Screen_Prototype::get_screen_options()}</li>
     *   <li>{@see ALP_Screen_Prototype::save_screen_options()}</li>
     *   <li>{@see ALP_Screen_Prototype::screen_options()}</li>
     * </ul>
     *
     ...
     * @return array
     * @since 1.0.0
     * @todo Make this automatic without need of writing own code - just use {@see ALP_Screen_Prototype::$options}.
     */
    ...
    protected function update_option( $key, $value ) {
        ...
        $options = ALP_Plugin::get_options();
        $need_update = false;
     ...
     * for `load-{$hookname}` hook (see {@see ALP_Screen_Prototype::admin_menu} 
     * for more details).
     * @return void
     * @since 1.0.0
     */
    ...
    /**
     * <p>Renders screen options form. Handler for `screen_layout_columns` filter (see {@see ALP_Screen_Prototype::screen_load}).</p>
     * @return void
     * @since 1.0.0
     * @todo It should be rendered automatically by using {@see ALP_Screen_Prototype::$options}.
    ...
    public function screen_options() {
        ...
        ob_start();
        $template = str_replace( ALP_SLUG . '-', '', "screen-{$this->slug}_options.phtml" );
        include( ALP_PATH . "partials/{$template}" );
        $output = ob_get_clean();
         ...
         */
        $output = apply_filters( ALP_SLUG . "_{$this->slug}_screen_options_form", $output );
        echo $output;
    }

    /**
     * <p>Save screen options. Action for `admin_init` hook (see {@see ALP_Screen_Prototype::init} for more details). Here is an example code how to save a screen option:</p>
     * <pre>
     * $user = get_current_user_id();
     *
     * if(
     ...
     * @return void
     * @since 1.0.0
     * @todo It should be done automatically by using {@see ALP_Screen_Prototype::$options} without need of writing own code.
     */
    ...
    public function render( $args = [] ) {
        ...
        ob_start();
        include( ALP_PATH . 'partials/screen-' . str_replace( ALP_SLUG . '-', '', $this->slug ) . '.phtml' );
        $output = ob_get_clean();
         ...
         * @param string $output Rendered HTML.
         */
        $output = apply_filters( ALP_SLUG . "_{$this->slug}", $output );
        echo $output;
    }
}
